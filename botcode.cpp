...
//Command to control the velocity of the bot by sending the required value of the velocity
...
/*
The Interrupt Service Routine will redirect the control here if an interrupt vector is generated by INT4 and increment the shaft count of the encoder on the left wheel
*/

// Interrupt 4 ISR definition 
ISR (INT4_vect)
{
	ShaftCountLeft++;
}
...
/*
The Interrupt Service Routine will redirect the control here if an interrupt vector is generated by INT5 and increment the shaft count of the encoder on the right wheel
*/
ISR(INT5_vect)
{
...
Logic : To reduce the error in rotation,  the bot is first rotated by the required angle in the main function and then the bot is rotated to the right by 10 degrees. One by one the bot turns to the left   5 degrees, each time and stores the maxima detected by the sharp sensor and from the left most position, the bot is rotated to this position, so that bot is always directed towards the next bal, to which it has to reach to.
...
{    
	...
	flag = flag*(-1);                                                 //To toggle the value
	if (flag == 1)	datahigh = data;
	else 
	{
		datalow = data;
		UDR0 = data; 				 //echo data back to PC so the x-bee transmitter on the laptop can be signalled to send the  the next bit , i.e. datahigh
	}	 
...
void init_devices()
{
	cli();                               //Clears the global interrupt
	uart0_init();      
            //Initializes the UART0 Port which is connected for the X-bee communication
...
int main()
{
	...
	while(1)
	{
		if (flag == 1) 		
		{
			...
			if (datahigh ==255 && datalow ==0)	continue;
			...
			{	theta= (volatile unsigned int) (convert_to_int(convert_to_binary(datalow)/100))+ (volatile unsigned int) ((datahigh)*64);
				...
				else        right_degrees(theta);          // since theta<0 rotate to right  
				stop();
